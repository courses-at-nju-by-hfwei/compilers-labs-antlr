...
​为了能够自定义打印的信息，我们需要对`notifiListeners(e)` 方法进行**Override**，一个合适的时机就是在生成CmmLexer对象的时候。所以上面得到Lexer对象的代码就会变为：
...
​在TODO1中，就可以调用`_input`的内部方法获取当前读取的错误的text以及错误发生的行号，最后生成错误语句，例如获取不能识别的text可以如下：

```java
// 代码参考自notifyListeners(LexerNoViableAltException e)源码，并非唯一获取方式
...
如果没有检测出词法错误，你就需要从之前获取的`tokenList`中遍历打印每一个`Token`的详细信息，即上述代码TODO2的内容，包括
...
其中，该对象的成员方法`getSymbolicName(int tokenType)`可以通过`tokenType`获取Token的`SymbolicName`，也就是需要识别出的Token的符号名称。

那么接下来需要做的就是关于如何获取Token的`tokenType`来获取符号名称

至此，我们就实现了错误词法单元报错，以及没有错误时的词法单元Token信息的打印

### 其他实现方式

除了从`CmmLexer`中获取tokenList，也可以将`CmmLexer`对象传入词法单元流中例如`CommonTokenStream` 当中
...
当然，获取Token的符号名称也可以不依赖`Vocabulary` 来获取，`CmmLexer.ruleNames`也存储了你在Antlr词法文件中写的一切Rule（自然也就包括了SymbolicNames）
...
⚠️ 本次实验在环境部署正确的情况下，不使用任何IDE也能完成实验。

⚠️ 若使用IDE例如idea等（方便查看antlr源码）编写代码，请新建项目，并在调试成功后，将实现的源代码文件放入本实验的src目录下，并且删除顶部可能会存在的package声明，以及无用的import，使用make调试成功后，进行`make submit`

⚠️ 请认真阅读本实验说明，实验一讲义，以及附录。
